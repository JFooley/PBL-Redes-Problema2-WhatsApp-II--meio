...
Partindo dessa base, a estrutura foi mantida porém com algumas alterações importantes. Para garantir que a sincronia na exibição fosse bem sucessidade, foi necessário implementar um sistema de confirmação em que cada nó, ao enviar uma mensagem, deve garantir que essa mensagem chegue para todos e posteriormente envie um pacote indicando aos nós que eles devem exibir a mensagem pois todos já confirmaram o recebimento. Porém, esse sistema apresenta uma grande fragilidade no modelo de falhas, pois, se algum pacote de se perder, toda a sincronia se perde, além de que é necessário diferenciar quando um nó não recebe uma mensagem por perda de pacote de quando ele não recebe por ter ficado offline. 
